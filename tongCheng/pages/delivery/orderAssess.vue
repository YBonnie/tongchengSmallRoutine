<template>
	<view class="page">
		<!-- 外卖员评价 -->
		<view class="takeaway_box" v-if="orderInfo.deliveryInfo">
			<view class="layout_box">
				<image :src="orderInfo.deliveryInfo.deliveryHeadImg" mode="aspectFill"></image>
				<view>
					<view class="title">跑腿员：{{ orderInfo.deliveryInfo.deliveryName }}</view>
					<view class="info">送达时间：{{ orderInfo.orderInfo.sendTimeStr }}</view>
				</view>
			</view>
			<!-- 评分星星 -->
			<view class="score_box"><text :class="{ active: deliveryScore >= item }" v-for="(item, index) of 5" :key="index" @click="onDeliveryScore(item)"></text></view>
		</view>
		<!-- 商品评价 -->
		<view class="orderAssess">
			<view class="layout_box">
				<image :src="orderInfo.shopLogo" mode="aspectFill"></image>
				<view>
					<view class="title">{{orderInfo.shopName}}</view>
					<view class="info"><text v-for="(item, index) of orderInfo.goodsList" :key="index">{{item.goodsName}}</text></view>
				</view>
			</view>
			<!-- 评分星星 -->
			<view class="score_box"><text :class="{ active: shopScore >= item }" v-for="(item, index) of 5" :key="index" @click="onShopScore(item)"></text></view>
			<!-- 上传照片 -->
...
export default {
	...
	methods: {
		// 星星评分
		onDeliveryScore(value) {
			this.deliveryScore = value;
		},
		// 星星评分
		onShopScore(value) {
...
.orderAssess {
	...
	.upload_box {
		display: flex;
		flex-wrap: wrap;
		padding: 50upx 0upx 50upx 61upx;
		.upload_img {
			position: relative;
			margin-right: 60upx;
			margin-top: 20upx;
