<template>
	<view>
		...
		<view class="menu_box">
			...
			<view :class="{ active: orderStatus == 1101 }" @click="onOrderStatus(1101)">待使用</view>
			<view :class="{ active: orderStatus == 1401 }" @click="onOrderStatus(1401)">待评价</view>
			<view :class="{ active: orderStatus == 1500 }" @click="onOrderStatus(1500)">已取消</view>
		</view>
	...
	methods: {
		...
		loadData() {
			if (this.pageNo <= this.pages) {
				...
				this.$http
					.post('api/order/tg/v1/list_data', httpData, {
						load: false
					})
					.then(res => {
