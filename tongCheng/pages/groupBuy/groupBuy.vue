<template>
	<view>
		...
		<view class="search_box" :class="{ active: searchActive }">
			...
			<view class="head_seach" @click="onPageJump('/pages/groupBuy/search')">点击输入关键字搜索</view>
		</view>
		<!-- banner -->
		...
		<swiper class="menu_swiper" :class="{ active: menuList.length == 1 && menuList[0].length <= 5 }" :indicator-dots="false"
		 :duration="1000" indicator-color="rgba(243,152,1,0.3)" indicator-active-color="#f39801">
			<swiper-item v-for="(item, index) of menuList" :key="index">
				<view class="nav_box">
					<view class="navs" v-for="(childItem, childIndex) of item" :key="childIndex" @click="onPageJump('/pages/groupBuy/shopList?typeId=' + childItem.objId)">
		...
		<view class="nearby_shops_box">
			...
			<view class="shop_list_box">
				<shop-item v-for="(item, index) of shopsLists" :key="index" :item="item"></shop-item>
			</view>
		</view>
		<load-more text="暂无店铺~" :height="40"></load-more>
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 轮播图点击
			onBanner(item) {
				if (item.jumpType == 1402) {
					uni.navigateTo({
						url: '/pages/delivery/shopDetail?objId=' + item.jumpRecord.id
					});
				} else if (item.jumpType == 1403) {
					uni.navigateTo({
						url: '/pages/groupBuy/shopDetail?objId=' + item.jumpRecord.id
					});
