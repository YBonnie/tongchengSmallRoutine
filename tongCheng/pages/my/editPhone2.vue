<template>
	<view class="page">
		...
		<view class="input_box"><input type="number" v-model="phone" placeholder="请输入新手机号" /></view>
		<view class="input_box">
			<input type="number" v-model="code" placeholder="请输入验证码" />
			<button @click="getCode">{{ codeText }}</button>
		</view>
		<view class="btn_box"><button @click="onSubmit">确定</button></view>
	</view>
</template>
<script>
...
export default {
	...
	methods: {
		...
		onSubmit() {
			...
			this.$http
				.post('api/mime/v1/check_new_phone', {
					phone: this.phone,
					code: this.code
				})
				.then(res => {
