...
export default {
	...
	methods: {
		...
		onSubmit() {
			...
			if (!(this.userInfo.wxSmallOpenId || this.userInfo.openId)) {
				uni.showToast({
					title: '数据丢失',
					icon: 'none'
				});
			...
			let httpData = {
				openId: this.userInfo.openId || this.userInfo.wxSmallOpenId,
				phone: this.phone,
				code: this.code,
				// #ifdef MP-WEIXIN
				type: 1201,
			...
			// #endif
			// #ifdef MP-WEIXIN
			if(this.chatScenesInfo.query.recommendCode){
				if(recommendCode){
					httpData.recommendCode = this.chatScenesInfo.query.recommendCode;
				}
			}
			// #endif
			this.$http
